...
const (
	...
	QueryPrevRoundGasFeeGroupByAddressSQL = "SELECT from_address, sum(gas_fee) as gas_fee FROM transaction_info WHERE status = 1 AND reward = 0 AND timestamp >= $1 AND timestamp < $2 GROUP BY from_address"
	QueryPrevRoundTotalGasFeeSQL          = "SELECT COALESCE(sum(gas_fee), 0) as gas_fee FROM transaction_info WHERE status = 1 AND reward = 0 AND timestamp >= $1 AND timestamp < $2"
	...
	QueryPrevRoundConfirmedTxsSQL         = "SELECT * FROM transaction_info WHERE status = 1 AND reward = 0 AND timestamp >= $1 AND timestamp < $2"
